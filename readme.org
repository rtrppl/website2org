...
*0.2.8*
- Added option to press the spacevar to scroll in =website2org-temp= + added option to call =website-url-to-org= from Elfeed (see below)
...
By adding a keybinding you are able to quickly open the current entry in a temporary website2org buffer (which can be closed by pressing "q").
